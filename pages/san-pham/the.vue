<template>
    <div>
        ...
        <a-modal title="Thêm thẻ mới" v-model="visible">
            <CategoryForm :is-category="false" @done="item => res.results.push(item)"/>
        </a-modal>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            async fetch() {
                ...
                this.res = await this.$axios.$get(`/warehouse/taxonomies/${params}`)
            },
            async onDelete(key) {
                await this.$axios.delete(`/warehouse/taxonomies/${key}/`)
                await this.fetch()
            },
            async onCellChange(key, dataIndex, value) {
                let data = {}
                data[dataIndex] = value
                await this.$axios.put(`/warehouse/taxonomies/${key}/`, data)
            },
        },
        created() {
            this.fetch()
